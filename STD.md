...
## 4. Test Scenarios (Current Coverage: 39 Tests)
...
### TS-07: Select-Options & Bundle Products ✨ NEW
**Objective**: Verify complex product configurations with dropdown options and bundle selections.
**Test Cases**:
  - Option selection validation: User selects from dropdown, error shown if empty.
  - Clear option selection: Reset button clears all selections.
  - Add to wishlist: Bundle product added to wishlist after option selection.
  - Add to cart workflows (5 parametrized scenarios):
    - Product 1, Option 1, Qty 1
    - Product 1, Option 2, Qty 2
    - Product 1, Option 3, Qty 5
    - Product 2, Option 1, Qty 1
    - Product 2, Option 2, Qty 3
**Files**: `tests/test_select_options_product.py`, `pages/product_page.py`
**Coverage**: 7 passing tests
**Bugs Detected**:
  - False Toast Bug: "Select options" button shows "added to cart" toast but product not actually added until reaching product detail page and clicking actual "Add to Cart" button.

## 🔍 Toast Notification Bug Detector
A global toast catcher was implemented to detect false or misleading toast messages across all add-to-cart operations. This automatically logs and reports discrepancies between UI feedback and actual cart state.

## 🚀 CI/CD Integration
**GitHub Actions Workflow** (`.github/workflows/playwright-tests.yml`):
- Automated testing on push/PR to main/master/develop branches
- Matrix testing: Python 3.11 & 3.12
- Generates Allure reports automatically
- Creates bug report artifacts
- Cross-platform support: Linux CI/CD environment
