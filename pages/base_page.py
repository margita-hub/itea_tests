...
class BasePage:
    ...
    @allure.step("Check all product titles contain '{keyword}'")
    ...
    @allure.step("Save cart with email {email}")
    ...
    @allure.step("Click and catch false toast: {action_name}")
    def click_and_catch_false_toast(self, button_locator, action_name: str) -> dict:
        ...
        if toast_appeared:
            ...
            if "add" in toast_text.lower() or "cart" in toast_text.lower():
                result['is_false_toast'] = True
        else:
            print("No toast message")
        return result